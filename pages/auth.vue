...
<template>
    ...
    <div class="flex flex-col justify-center items-center w-screen h-screen bg-[#b0b8b6] bg-cover bg-center">
        <NavBar />
        <Card v-if="mode === 'login'"
            class="mx-auto min-h-[50vh] w-full max-w-sm sm:max-w-md md:max-w-lg lg:max-w-xl xl:max-w-2xl">
            <template #title>Welcome Back!</template>
            <template #subtitle>Please enter your information</template>
            <template #content>
                <div class="m-5">
                    <Form v-slot="$form" :resolver="login_resolver" :initialValues="initialValues"
                        @submit="(e) => onLoginFormSubmit(e)" class="flex justify-center flex-col gap-4">
                        ...
                        <div class="flex flex-col gap-1">
                            <Password name="password" placeholder="Password" toggleMask :feedback="false" class="w-full"
                                inputClass="w-full" />
                    ...
                    <div class="m-5 flex justify-center flex-col gap-4 text-center">
                        <p>
                            Don't have an account? <RouterLink class="text-blue-600 hover:underline cursor-pointer"
                                to="/auth?mode=signup">Sign Up!</RouterLink>
                        </p>
                        <p>
                            <RouterLink class="text-blue-600 hover:underline cursor-pointer" to="/auth?mode=forgot">
        ...
        <Card v-else-if="mode === 'signup'"
            class="mx-auto h-[50vh] w-full max-w-sm sm:max-w-md md:max-w-lg lg:max-w-xl xl:max-w-2xl">
            <template #title>Welcome!</template>
            <template #subtitle>Please enter your information</template>
            <template #content>
                <div class="m-5">
                    <Form v-slot="$form" :resolver="signup_resolver" :initialValues="initialSignUpValues"
                        @submit="(e) => onSignUpFormSubmit(e)" class="flex justify-center flex-col gap-4">
                        <div class="flex flex-col gap-1">
                            <InputText name="email" type="text" placeholder="Email" />
                            <Message v-if="$form.email?.invalid" severity="error" size="small" variant="simple">{{
                                $form.email.error?.message }}</Message>
                        ...
                        <div class="flex flex-col gap-1">
                            <Password name="password" placeholder="Password" toggleMask class="w-full"
                                inputClass="w-full" />
                            <Message v-if="$form.password?.invalid" severity="error" size="small" variant="simple">{{
                                $form.password.error?.message }}</Message>
                        </div>
                    ...
                    <div class="m-5 flex justify-center flex-col gap-4 text-center">
                        <p>
                            Have an account? <RouterLink class="text-blue-600 hover:underline cursor-pointer"
                                to="/auth?mode=login">Log In!</RouterLink>
                        </p>
                    </div>
                </div>
            </template>
        </Card>
        <Card v-else-if="mode === 'forgot'"
            class="mx-auto h-[40vh] w-full max-w-sm sm:max-w-md md:max-w-lg lg:max-w-xl xl:max-w-2xl">
            <template #title>Forgot your password?</template>
            <template #subtitle>Lets get that fixed!</template>
            <template #content>
                <div class="m-5">
                    <Form v-slot="$form" :resolver="forgot_resolver" :initialValues="initialForgotValues"
                        @submit="(e) => onForgotFormSubmit(e)" class="flex justify-center flex-col gap-4">
                        <div class="flex flex-col gap-1">
                            <InputText name="email" type="text" placeholder="Email" />
                            <Message v-if="$form.email?.invalid" severity="error" size="small" variant="simple">{{
                                $form.email.error?.message }}</Message>
                    ...
                    <div class="m-5 flex justify-center flex-col gap-4 text-center">
                        <p>
                            Changed your mind? <RouterLink class="text-blue-600 hover:underline cursor-pointer"
                                to="/auth?mode=login">Return To Log In!</RouterLink>
                        </p>
                    </div>
                </div>
            </template>
        ...
        <Card v-else
            class="flex flex-col justify-center items-center mx-auto h-[50vh] w-full max-w-sm sm:max-w-md md:max-w-lg lg:max-w-xl xl:max-w-2xl">
            <template #content>
